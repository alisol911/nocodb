...
const fetchMissingTableMetas = async (localTables: TableType[]) => {
  ...
  const filteredTables = localTables.filter((t: TableType) => !metas.value[t.id!] && t.base_id === props.sourceId)
...
const filteredTables = computed(() =>
  tables.value.filter((t: TableType) =>
    props?.sourceId
      ? t.base_id === props.sourceId
      : t.base_id === sources.value?.filter((source: SourceType) => source.enabled)[0]?.id,
  ),
)
